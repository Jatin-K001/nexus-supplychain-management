...
export function SupervisorProjectDetail() {
  ...
  const { data: project } = useQuery<Project>({ queryKey: ['project', projectId], queryFn: () => api.get(`/api/projects/${projectId}`) });
  ...
  return (
    <div>
      <Breadcrumb parts={['MY PROJECTS', project?.name?.toUpperCase() ?? '']} />
      ...
      <PhaseListCard
        phases={phases ?? []}
        linkTo={(p) => `/supervisor/projects/${projectId}/phases/${p.id}`}
        highlightInProgress
        disableLocked
      />
    </div>
